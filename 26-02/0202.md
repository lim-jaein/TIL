...
4. 비즈니스 로직 실행
    ...
    힙 내부의 영속성 컨텍스트에 flush 전의 데이터가 저장된다
    
    ```sql
    [ Heap ]
    EntityManager (영속성 컨텍스트 포함)
    ├── 1차 캐시 (Map)
    └── 쓰기 지연 SQL 저장소 (List)
    ```
...
- **현재 진행 중인 트랜잭션이 있으면, 그 안에 중첩(Nested) 트랜잭션을 만든다.**
- 현재 진행 중인 트랜잭션이 없으면, REQUIRED 처럼 새로운 트랜잭션을 시작한다.
- 내부적으로는 저장점(Savepoint)을 사용해, 중첩 영역에서 에러가 발생하면 해당 저장점까지만 부분 롤백하고 메인 트랜잭션은 유지된다.

### Propagation.SUPPORTED

- 현재 진행 중인 트랜잭션이 있다면 참여하고, 없으면 트랜잭션 없이 실행한다

### Propagation.NOT_SUPPORTED

- 현재 진행 중인 트랜잭션이 있다면 중단하고, 트랜잭션 없이 실행한다
- 실행이 끝난 후 다시 기존 트랜잭션을 재개한다

### Propagation.MANDATORY

- 반드시 트랜잭션이 있어야하며, 없을 경우 예외가 발생한다.

### Propagation.NEVER

- 반드시 트랜잭션이 없어야하며, 있을 경우 예외가 발생한다.
